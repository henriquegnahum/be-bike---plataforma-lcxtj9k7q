export const MOCK_CRM_LEADS = [
  {
    id: '1',
    name: 'Carlos Silva',
    phone: '11988881111',
    stage: 'Leads',
    source: 'Instagram',
    type: 'Entregador 99',
    cpf: '123.456.789-00',
    rg: '12.345.678-9',
    uf: 'SP',
    gender: 'Masculino',
    dob: '1990-05-15',
    parentsNames: 'Maria Silva, José Silva',
    civilStatus: 'Solteiro',
    address: 'Rua das Flores, 123 - Centro',
    cep: '01000-000',
    contractStatus: 'Vigente',
  },
  {
    id: '2',
    name: 'Ana Oliveira',
    phone: '11977772222',
    stage: 'Contacted',
    source: 'Referral',
    type: 'B2B',
    cpf: '234.567.890-11',
    rg: '23.456.789-0',
    uf: 'RJ',
    gender: 'Feminino',
    dob: '1988-10-22',
    parentsNames: 'Sonia Oliveira',
    civilStatus: 'Casada',
    address: 'Av Paulista, 1000',
    cep: '01310-100',
    contractStatus: 'Em Análise',
  },
  {
    id: '3',
    name: 'Pedro Santos',
    phone: '11966663333',
    stage: 'Analysis',
    source: 'Website',
    type: 'Entregador',
  },
  {
    id: '4',
    name: 'João Costa',
    phone: '11955554444',
    stage: 'Negotiation',
    source: 'Organic',
    type: 'Outros',
  },
  {
    id: '5',
    name: 'Maria Lima',
    phone: '11944445555',
    stage: 'Signed/Onboarding',
    source: 'Instagram',
    type: 'Entregador 99',
  },
]

export const MOCK_BIKES = [
  {
    id: '1',
    chassi: '9C21A4B2C3D4E5F6G',
    status: 'Ativa',
    model: 'BeBike City Pro',
    version: '2024 V1',
    brand: 'BeBike',
    size: 'M',
    color: 'Laranja',
    trackerNum: 'TRK-9001',
    batteryNum: 'BAT-1102',
    lockCode: '8842',
    mileage: 1250,
    lastRevisionKm: 0,
    deliverer: 'Carlos Silva',
  },
  {
    id: '2',
    chassi: '9C22B5C3D4E5F6G7H',
    status: 'Oficina',
    model: 'BeBike Cargo',
    version: '2023 V2',
    brand: 'BeBike',
    size: 'G',
    color: 'Preto',
    trackerNum: 'TRK-9055',
    batteryNum: 'BAT-1150',
    lockCode: '1123',
    mileage: 3400,
    lastRevisionKm: 2500,
    deliverer: null,
  },
  {
    id: '3',
    chassi: '9C23C6D4E5F6G7H8I',
    status: 'Disponível',
    model: 'BeBike City Pro',
    version: '2024 V1',
    brand: 'BeBike',
    size: 'P',
    color: 'Laranja',
    trackerNum: 'TRK-9088',
    batteryNum: 'BAT-1200',
    lockCode: '9900',
    mileage: 150,
    lastRevisionKm: 0,
    deliverer: null,
  },
]

export const MOCK_BIKE_OS = [
  {
    id: 'OS-1029',
    date: '2024-03-15',
    description: 'Troca de pastilha de freio',
    cost: 45.0,
    status: 'Completed',
  },
  {
    id: 'OS-1045',
    date: '2024-04-02',
    description: 'Reparo de pneu furado',
    cost: 25.0,
    status: 'Completed',
  },
]

export const MOCK_BIKE_PARTS = [
  { id: 'P-01', name: 'Pastilha de Freio Cerâmica', stock: 45, status: 'Good' },
  { id: 'P-02', name: 'Pneu All-Weather 26"', stock: 12, status: 'Low' },
  { id: 'P-03', name: 'Lubrificante Corrente 500ml', stock: 8, status: 'Critical' },
]

export const MOCK_DELIVERERS = [
  {
    id: '1',
    name: 'Carlos Silva',
    cpf: '123.456.789-00',
    status: 'Active',
    contractStatus: 'Adimplente',
    trips: 145,
    rating: 4.8,
    activeBike: '9C21A4B2C3D4E5F6G',
    joinDate: '2023-11-10',
    phone: '11988881111',
  },
  {
    id: '2',
    name: 'Ana Oliveira',
    cpf: '234.567.890-11',
    status: 'Onboarding',
    contractStatus: 'Em Análise',
    trips: 0,
    rating: 0,
    activeBike: null,
    joinDate: '2024-04-25',
    phone: '11977772222',
  },
  {
    id: '3',
    name: 'Pedro Santos',
    cpf: '345.678.901-22',
    status: 'In Debt',
    contractStatus: 'Inadimplente',
    trips: 412,
    rating: 4.2,
    activeBike: '9C23C6D4E5F6G7H8I',
    joinDate: '2023-08-05',
    phone: '11966663333',
  },
]

export const MOCK_DELIVERER_LOGS = [
  {
    id: 'L-1',
    date: '2024-04-24 10:30',
    type: 'Communication',
    note: 'Lembrete de pagamento via WhatsApp.',
  },
  {
    id: 'L-2',
    date: '2024-04-10 14:15',
    type: 'Contract',
    note: 'Novo contrato de leasing assinado (ZapSign).',
  },
]

export const MOCK_CLOUD_HUMANS_TICKETS = [
  {
    id: 'CH-8821',
    subject: 'Problema Login App',
    user: 'Ana Oliveira',
    status: 'Open',
    priority: 'High',
    time: '10m ago',
  },
]

export const taskColumns = [
  {
    id: 'todo',
    title: 'A Fazer (O.S.)',
    tasks: [{ id: 'OS-1100', chassi: '9C21A', desc: 'Revisão 2500km' }],
  },
  { id: 'doing', title: 'Em Manutenção', tasks: [] },
  { id: 'done', title: 'Finalizado', tasks: [] },
]

export const inventoryItems = [
  { id: '1', name: 'Pneu Aro 26', hub: 'SP-Centro', usage: 'Alto', min: 20, current: 15 },
  { id: '2', name: 'Câmara de Ar', hub: 'SP-Centro', usage: 'Alto', min: 50, current: 80 },
]

export const pendingApprovals = [
  {
    id: 'APP-091',
    team: 'Financeiro',
    type: 'Estorno',
    desc: 'Devolução de caução',
    value: 'R$ 150,00',
    requester: 'Op. Hub Sul',
  },
]
